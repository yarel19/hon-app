...
assert(theme.includes('.stitch-history-balance-point{text-align:center'),'יתרות תחילת וסוף החודש אינן מיושרות למרכז במחשב');
assert(theme.includes('.stitch-history-category-grid{grid-template-columns:repeat(3,minmax(0,1fr))}'),'סיכום הקטגוריות אינו מוצג בשלושה טורים במחשב');
...
console.log('HON Stitch v30 passed budget and month-close target comparisons, centered history balances, three-column history categories, safe overview layout, existing-data preservation, and all interface regression checks.');
