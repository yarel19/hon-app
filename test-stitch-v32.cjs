...
assert(d.querySelector('.stitch-dashboard-rail .stitch-balance-grid'),'כרטיסי היתרות אינם בעמודה העצמאית של הסקירה');
assert(d.querySelector('.stitch-dashboard-rail')?.tagName==='DIV','עמודת הסקירה המשנית עדיין משתמשת בתגית aside ומתנגשת בתפריט המחשב');
assert(d.querySelectorAll('body > aside').length===1,'נוצר תפריט צד כפול שמסתיר את הסקירה במחשב');
assert(theme.includes('grid-template-areas:"main rail"'),'פריסת המחשב עדיין יוצרת רווח בין אזורי הסקירה');
...
assert(d.querySelectorAll('.stitch-cashflow-kpis .stitch-target-compare').length===2,'ההשוואות בתקציב אינן מופיעות רק בהכנסות ובהוצאות');
assert(d.querySelector('.stitch-cashflow-kpis .income .stitch-target-compare')?.textContent.includes('מתחת ליעד ההכנסה'),'הכנסה בפועל אינה מושווית ליעד שהוגדר');
assert(d.querySelector('.stitch-cashflow-kpis .income .stitch-target-compare')?.textContent.includes('% מהיעד'),'יחס ההכנסה באחוזים מול היעד חסר');
assert(d.querySelector('.stitch-cashflow-kpis .out .stitch-target-compare')?.textContent.includes('בתקציב הקטגוריות'),'ההוצאות אינן מושוות לתקציב הקטגוריות');
assert(d.querySelector('.stitch-cashflow-kpis .out .stitch-target-compare')?.textContent.includes('מתוך'),'סכום הוצאות הקטגוריות מול התקציב חסר');
assert(!d.querySelector('.stitch-cashflow-kpis .balance .stitch-target-compare'),'מתחת ליתרה לניהול עדיין מופיע הסבר מיותר');
...
visit('history');
assert(d.querySelector('.stitch-history-page'),'דף ההיסטוריה החודשית חסר');
assert(d.querySelector('.stitch-history-hero')?.textContent.includes('חודשי'),'סיכום הרווח או ההפסד החודשי חסר');
assert(d.querySelectorAll('.stitch-history-balance-point').length===2,'יתרות תחילת וסוף החודש אינן מוצגות');
assert(d.querySelector('.stitch-history-balance-point')?.textContent.includes('עו״ש'),'פירוט העו״ש בהיסטוריה חסר');
assert(d.querySelectorAll('.stitch-history-kpis article').length===4,'מדדי החודש החשובים אינם מסודרים');
assert(d.querySelector('.stitch-history-category-grid')?.textContent.includes('מזון וקניות'),'סיכום הקטגוריות החודשי חסר');
assert(theme.includes('.stitch-history-balance-point>strong[dir="ltr"]{display:block;width:100%;text-align:center!important}'),'סכומי יתרות תחילת וסוף החודש אינם ממורכזים בפועל');
assert(theme.includes('.stitch-history-content{display:grid;grid-template-columns:minmax(0,1fr)'),'אזור סיכום הקטגוריות אינו מנצל את מלוא רוחב הדף');
assert(theme.includes('.stitch-history-category-grid{grid-template-columns:repeat(4,minmax(0,1fr))}'),'סיכום הקטגוריות אינו מוצג בארבעה טורים במסך רחב');
assert(d.querySelector('.stitch-history-insights')?.tagName==='DIV','תובנות ההיסטוריה עדיין מתנגשות בתפריט הצד של המחשב');
assert(theme.includes('.stitch-history-insights article>span>.material-symbols-outlined'),'האייקון בתובנות ההיסטוריה אינו מקובע בתוך הריבוע');
assert(d.querySelector('[data-history-month]'),'המעבר בין חודשי ההיסטוריה חסר');

visit('review');
assert(d.querySelector('.stitch-review-card'),'מסך סגירת החודש חסר');
assert(d.querySelectorAll('.stitch-review-kpis .stitch-target-compare').length===3,'סגירת החודש אינה מציגה יחס מול היעדים בכל המדדים');
assert(d.querySelector('.stitch-review-card')?.textContent.includes('סיכום פיננסי מול היעדים'),'סיכום סגירת החודש אינו מדגיש את ההשוואה לתוכנית');

visit('settings');
assert(d.querySelector('#categorySorter'),'ניהול הקטגוריות חסר');
assert(d.querySelector('.stitch-sync-protection'),'כרטיס הגנת הסנכרון חסר');
assert(d.querySelector('#cloudVersions'),'גישה לגרסאות קודמות בענן חסרה');
...
console.log('HON Stitch v32 passed category-budget variance in actual outflow, clean management balance, contained history insight icon, full-width history, existing-data preservation, and all interface regression checks.');
