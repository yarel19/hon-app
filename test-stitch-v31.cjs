...
assert(d.querySelectorAll('.stitch-cashflow-kpis .stitch-target-compare').length===3,'חסרה השוואה מול היעד במדדי התקציב');
...
assert(d.querySelector('.stitch-cashflow-kpis .out .stitch-target-compare')?.textContent.includes('במסגרת'),'ההוצאות אינן מושוות למסגרת החודשית');
...
console.log('HON Stitch v31 passed true centered history amounts, full-width four-column category archive, isolated monthly insights, target comparisons, existing-data preservation, and all interface regression checks.');
