...
function stitchSaveCategoryAssessment(card,categoryRows){
 ...
 let previous=(record.categoryAssessments||[]).find(x=>x.category===category)||{},assessment={...previous,category,rating:card.querySelector('input[type="radio"]:checked')?.value||'',note:card.querySelector('textarea').value.trim(),target:stats.target||0,spent:stats.spent||0,difference:stats.difference||0,transactions:stats.transactions||0,budgetChange:stats.budgetChange||{count:0,net:0,increased:0,decreased:0},updatedAt:new Date().toISOString()};
...
function stitchRefreshCategoryAssessment(card,categoryRows,moveCompleted=false){
 let complete=!!card.querySelector('input[type="radio"]:checked'),state=card.querySelector('.stitch-category-save-state');card.classList.toggle('completed',complete);if(state)state.textContent=complete?'נשמר אוטומטית':'טיוטה נשמרה';
 if(complete&&moveCompleted)card.parentElement?.append(card);
 let count=$$('[data-category-assessment]').filter(item=>item.querySelector('input:checked')).length,progress=$('.stitch-category-review-progress');if(progress)progress.textContent=`${count} מתוך ${categoryRows.length} סווגו`
}
function stitchCategoryAssessmentCard(row,saved={}){
 ...
 let choices=[['excellent','verified','מצוין'],['improve','build_circle','דרוש שיפור'],['extreme','error','חריגה קיצונית']],changes=stitchCategoryChangeEntries(row.id,selectedMonth),completed=!!saved.rating;
 return `<article class="stitch-category-assessment-card ${state} ${completed?'completed':''}" data-category-assessment="${esc(row.id)}"><header><span class="stitch-category-assessment-icon" style="--assessment-color:${row.color}">${stitchIcon(row.icon||'receipt_long')}</span><div><h3>${esc(row.name)}</h3><p class="${state}">${status}</p></div><div class="stitch-category-assessment-head-actions">${changes.length?`<button type="button" data-action="categoryChanges" data-id="${esc(row.id)}" title="הצגת כל השינויים">${stitchIcon('history')}<span>${changes.length}</span></button>`:''}<strong>${money(row.spent)}</strong></div></header><div class="stitch-category-assessment-facts"><span><small>יעד חודשי</small><b>${money(row.target)}</b></span><span><small>הוצאה בפועל</small><b>${money(row.spent)}</b></span><span><small>תנועות</small><b>${row.transactions}</b></span></div><div class="stitch-category-assessment-budget">${stitchBudgetChangeLine(row.budgetChange)}</div><fieldset><legend>איך היה החודש בקטגוריה הזו?</legend><div class="stitch-category-rating">${choices.map(([value,icon,label])=>`<label class="${value}"><input type="radio" name="rating_${esc(row.id)}" value="${value}" ${saved.rating===value?'checked':''} required><span>${stitchIcon(icon)}<b>${label}</b></span></label>`).join('')}</div></fieldset><label class="stitch-category-note"><span>הערה אישית <small class="stitch-category-save-state">${completed?'נשמר אוטומטית':saved.note?'טיוטה נשמרה':'אופציונלי'}</small></span><textarea rows="2" maxlength="300" placeholder="מה עבד, מה השתנה ומה כדאי לזכור לחודש הבא?">${esc(saved.note||'')}</textarea></label></article>`
}

review=function(){
 heading('סגירת חודש','אימות, תובנות והחלטה אחת לחודש הבא');
 let t=totals(selectedMonth),rows=[...monthTx(selectedMonth)].sort((a,b)=>(b.date||'').localeCompare(a.date||'')),unreviewed=rows.filter(x=>!x.reviewed),existing=db.reviews.find(r=>r.month===selectedMonth)||{},categoryRows=monthlyCategoryStats(selectedMonth),savedAssessments=new Map((existing.categoryAssessments||[]).map(x=>[x.category,x]));categoryRows.sort((a,b)=>Number(!!savedAssessments.get(a.id)?.rating)-Number(!!savedAssessments.get(b.id)?.rating));let assessedCount=categoryRows.filter(x=>savedAssessments.get(x.id)?.rating).length,steps=['סיכום פיננסי','אימות עסקאות','הערכת קטגוריות','רפלקציה'],body='',targets=stitchMonthlyTargets(selectedMonth),actualOut=t.expense+t.saving+t.debt,actualBalance=t.income-actualOut;
 ...
 $$('.stitch-category-rating input').forEach(input=>input.onchange=()=>{let card=input.closest('[data-category-assessment]');stitchSaveCategoryAssessment(card,categoryRows);stitchRefreshCategoryAssessment(card,categoryRows,true)});
