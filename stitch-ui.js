...
function stitchSaveCategoryAssessment(card,categoryRows){
 ...
 let previous=(record.categoryAssessments||[]).find(x=>x.category===category)||{},completed=card.dataset.completed==='true',now=new Date().toISOString(),assessment={...previous,category,rating:card.querySelector('.stitch-category-rating input:checked')?.value||'',note:card.querySelector('textarea').value.trim(),completed,completedAt:completed?(previous.completedAt||now):'',target:stats.target||0,spent:stats.spent||0,difference:stats.difference||0,transactions:stats.transactions||0,budgetChange:stats.budgetChange||{count:0,net:0,increased:0,decreased:0},updatedAt:now};
...
review=function(){
 ...
 $$('.stitch-category-rating input').forEach(input=>input.onchange=()=>{let card=input.closest('[data-category-assessment]');if(input.checked)card.querySelectorAll('.stitch-category-rating input').forEach(other=>{if(other!==input)other.checked=false});card.dataset.completed='false';stitchSaveCategoryAssessment(card,categoryRows);stitchRefreshCategoryAssessment(card,categoryRows,true)});
 ...
 $$('[data-category-finish]').forEach(button=>button.onclick=()=>{let card=button.closest('[data-category-assessment]');if(!card.querySelector('.stitch-category-rating input:checked'))return toast('יש לבחור דירוג לפני שמסיימים');card.dataset.completed='true';stitchSaveCategoryAssessment(card,categoryRows);stitchRefreshCategoryAssessment(card,categoryRows,true)});
 if($('#categoryAssessmentForm'))$('#categoryAssessmentForm').onsubmit=e=>{e.preventDefault();let record=db.reviews.find(r=>r.month===selectedMonth),currentAssessments=record?.categoryAssessments||[],categoryAssessments=$$('[data-category-assessment]').map(card=>{let category=card.dataset.categoryAssessment,stats=categoryRows.find(x=>x.id===category)||{},previous=currentAssessments.find(x=>x.category===category)||{},completed=card.dataset.completed==='true';return{...previous,category,rating:card.querySelector('.stitch-category-rating input:checked')?.value||'',note:card.querySelector('textarea').value.trim(),completed,completedAt:completed?(previous.completedAt||new Date().toISOString()):'',target:stats.target||0,spent:stats.spent||0,difference:stats.difference||0,transactions:stats.transactions||0,budgetChange:stats.budgetChange||{count:0,net:0,increased:0,decreased:0}}});if(categoryAssessments.some(x=>!x.rating||!x.completed))return toast('יש ללחוץ סיימתי בכל קטגוריה');if(record)record.categoryAssessments=categoryAssessments;else db.reviews.push({month:selectedMonth,categoryAssessments});persist();reviewStep=4;render();toast('הערכת הקטגוריות נשמרה')};
