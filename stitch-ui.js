...
monthlyHistory=function(){
 ...
 $('#view').innerHTML=`<section class="stitch-page stitch-history-page">
  ...
  <div class="stitch-history-content">
   ...
   <div class="stitch-history-insights" role="complementary" aria-label="תובנות החודש"><article><span>${stitchIcon('monitoring')}</span><div><small>הקטגוריה הגדולה</small><b>${data.topCategory?esc(data.topCategory.name):'אין עדיין'}</b><strong>${data.topCategory?money(data.topCategory.spent):'0 ₪'}</strong></div></article><article class="net-worth"><span>${stitchIcon('account_balance_wallet')}</span><div><small>${worth.live?'הון עצמי עכשיו':'הון עצמי בסוף החודש'}</small><b>${money(worth.closing)}</b><strong class="${worth.change<0?'down':worth.change>0?'up':''}">${worthChange}</strong></div></article>${reviewText?`<article class="decision"><span>${stitchIcon('task_alt')}</span><div><small>החלטת החודש</small><b>${esc(reviewText)}</b></div></article>`:''}</div>
  </div>
 </section>`;
