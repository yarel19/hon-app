...
monthlyHistory=function(){
 ...
 let index=months.indexOf(selectedMonth),data=monthlyHistoryData(selectedMonth),closed=selectedMonth<monthKey()&&!!(data.review||data.snapshot),historyStatus=selectedMonth===monthKey()?'נתונים חיים':closed?'חודש סגור ומתועד':'סיכום מחושב',maxCategory=Math.max(1,...data.categories.map(x=>x.spent)),resultLabel=data.result>=0?'רווח חודשי':'הפסד חודשי',recentMonths=months.slice(-7).reverse(),reviewText=data.review?.decision||data.review?.lesson||data.review?.win||'';
 $('#view').innerHTML=`<section class="stitch-page stitch-history-page">
  ...
  <div class="stitch-history-kpis"><article><span class="income">${stitchIcon('south_west')}</span><small>הכנסות</small><strong>${money(data.totals.income)}</strong></article><article class="expense-only"><span class="out">${stitchIcon('north_east')}</span><small>כל מה שיצא</small><strong>${money(data.expenseOnly)}</strong></article><article><span class="saving">${stitchIcon('savings')}</span><small>חיסכון והפקדות</small><strong>${money(data.totals.saving)}</strong></article><article><span class="count">${stitchIcon('receipt_long')}</span><small>מספר תנועות</small><strong>${data.transactionCount}</strong></article></div>
  <div class="stitch-history-content">
   ...
   ${reviewText?`<div class="stitch-history-insights" role="complementary" aria-label="החלטת החודש"><article class="decision"><span>${stitchIcon('task_alt')}</span><div><small>החלטת החודש</small><b>${esc(reviewText)}</b></div></article></div>`:''}
  </div>
 </section>`;
...
function stitchCategoryChangeEntries(categoryId,m=selectedMonth){
 let budget=db.budgetChangeLog.filter(x=>x.month===m&&x.category===categoryId).map(x=>({kind:'budget',createdAt:x.createdAt||'',title:x.reason==='iron'?'החלת תקציב ברזל':x.reason==='copy'?'העתקה מהחודש הקודם':'עריכת התקציב החודשי',detail:`מ-${money(x.before)} ל-${money(x.after)}`,change:roundBudgetAmount(x.change)}));
 let flex=flexFundEntries(m).flatMap(x=>x.type==='collect'&&x.sourceCategory===categoryId?[{kind:'collect',createdAt:x.createdAt||'',title:'איסוף לקופת הגמישות',detail:`נאספו מהקטגוריה ${money(x.amount)}`,change:-roundBudgetAmount(x.amount)}]:x.type==='allocate'&&x.targetCategory===categoryId?[{kind:'allocate',createdAt:x.createdAt||'',title:'חלוקה מקופת הגמישות',detail:`נוספו לקטגוריה ${money(x.amount)}`,change:roundBudgetAmount(x.amount)}]:[]);
 return [...budget,...flex].sort((a,b)=>(b.createdAt||'').localeCompare(a.createdAt||''))
}
function stitchCategoryChangeDate(value){
 let date=new Date(String(value||'').slice(0,24));return Number.isNaN(date.getTime())?'במהלך החודש':date.toLocaleString('he-IL',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})
}
function stitchOpenCategoryChanges(categoryId){
 let category=cat(categoryId),entries=stitchCategoryChangeEntries(categoryId,selectedMonth);if(!entries.length)return toast('לא נרשמו שינויים בקטגוריה הזו');
 modal(`השינויים ב-${category.name}`,`<div class="stitch-category-changes-intro"><span>${stitchIcon(categoryIconName(category))}</span><div><b>${monthName(selectedMonth)}</b><small>${entries.length} ${entries.length===1?'שינוי מתועד':'שינויים מתועדים'}</small></div></div><div class="stitch-category-changes-list">${entries.map(entry=>`<article class="${entry.change<0?'decrease':'increase'}"><span>${stitchIcon(entry.kind==='collect'?'move_to_inbox':entry.kind==='allocate'?'outbox':'edit_note')}</span><div><b>${esc(entry.title)}</b><small>${esc(entry.detail)} · ${stitchCategoryChangeDate(entry.createdAt)}</small></div><strong dir="ltr">${signedMoney(entry.change)}</strong></article>`).join('')}</div><div class="form-actions full"><button type="button" class="primary" id="closeCategoryChanges">סגירה</button></div>`,()=>{});
 $('#closeCategoryChanges').onclick=()=>$('#modal').hidden=true
}
actions.categoryChanges=stitchOpenCategoryChanges;

function stitchSaveCategoryAssessment(card,categoryRows){
 let category=card.dataset.categoryAssessment,stats=categoryRows.find(x=>x.id===category)||{},record=db.reviews.find(r=>r.month===selectedMonth);if(!record){record={month:selectedMonth,categoryAssessments:[]};db.reviews.push(record)}
 let previous=(record.categoryAssessments||[]).find(x=>x.category===category)||{},completed=card.dataset.completed==='true'&&card.dataset.completionSource==='button',now=new Date().toISOString(),assessment={...previous,category,rating:card.querySelector('.stitch-category-rating input:checked')?.value||'',note:card.querySelector('textarea').value.trim(),completed,completionSource:completed?'button':'',completedAt:completed?(previous.completedAt||now):'',target:stats.target||0,spent:stats.spent||0,difference:stats.difference||0,transactions:stats.transactions||0,budgetChange:stats.budgetChange||{count:0,net:0,increased:0,decreased:0},updatedAt:now};
 record.categoryAssessments=[...(record.categoryAssessments||[]).filter(x=>x.category!==category),assessment];persist();return assessment
}
function stitchRefreshCategoryAssessment(card,categoryRows,moveCompleted=false){
 let complete=card.dataset.completed==='true',hasRating=!!card.querySelector('.stitch-category-rating input:checked'),state=card.querySelector('.stitch-category-save-state'),finish=card.querySelector('[data-category-finish]');card.classList.toggle('completed',complete);if(state)state.textContent=complete?'הושלם':hasRating?'הדירוג נשמר':'טיוטה נשמרה';
 if(finish){finish.disabled=complete;finish.innerHTML=`${stitchIcon(complete?'task_alt':'done')}<span>${complete?'הושלם':'סיימתי'}</span>`}
 if(moveCompleted&&card.parentElement){if(complete)card.parentElement.append(card);else{let firstCompleted=card.parentElement.querySelector('.stitch-category-assessment-card.completed');if(firstCompleted&&firstCompleted!==card)card.parentElement.insertBefore(card,firstCompleted)}}
 let count=$$('[data-category-assessment]').filter(item=>item.dataset.completed==='true').length,progress=$('.stitch-category-review-progress');if(progress)progress.textContent=`${count} מתוך ${categoryRows.length} הושלמו`
}
function stitchCategoryAssessmentCard(row,saved={}){
 ...
 let choices=[['excellent','verified','מצוין'],['improve','build_circle','דרוש שיפור'],['extreme','error','חריגה קיצונית']],changes=stitchCategoryChangeEntries(row.id,selectedMonth),completed=saved.completed===true;
 return `<article class="stitch-category-assessment-card ${state} ${completed?'completed':''}" data-category-assessment="${esc(row.id)}" data-completed="${completed}" data-completion-source="${completed?'button':''}"><header><span class="stitch-category-assessment-icon" style="--assessment-color:${row.color}">${stitchIcon(row.icon||'receipt_long')}</span><div><h3>${esc(row.name)}</h3><p class="${state}">${status}</p></div><div class="stitch-category-assessment-head-actions">${changes.length?`<button type="button" data-action="categoryChanges" data-id="${esc(row.id)}" title="הצגת כל השינויים">${stitchIcon('history')}<span>${changes.length}</span></button>`:''}<strong>${money(row.spent)}</strong></div></header><div class="stitch-category-assessment-facts"><span><small>יעד חודשי</small><b>${money(row.target)}</b></span><span><small>הוצאה בפועל</small><b>${money(row.spent)}</b></span><span><small>תנועות</small><b>${row.transactions}</b></span></div><fieldset><legend>איך היה החודש בקטגוריה הזו?</legend><div class="stitch-category-rating" role="radiogroup">${choices.map(([value,icon,label])=>`<label class="${value}"><input type="checkbox" name="rating_${esc(row.id)}" value="${value}" ${saved.rating===value?'checked':''}><span>${stitchIcon(icon)}<b>${label}</b></span></label>`).join('')}</div></fieldset><label class="stitch-category-note"><span>הערה אישית <small class="stitch-category-save-state">${completed?'הושלם':saved.rating?'הדירוג נשמר':saved.note?'טיוטה נשמרה':'אופציונלי'}</small></span><textarea rows="2" maxlength="300" placeholder="מה עבד, מה השתנה ומה כדאי לזכור לחודש הבא?">${esc(saved.note||'')}</textarea></label><div class="stitch-category-assessment-footer"><button type="button" class="stitch-category-finish" data-category-finish ${completed?'disabled':''}>${stitchIcon(completed?'task_alt':'done')}<span>${completed?'הושלם':'סיימתי'}</span></button></div></article>`
}

review=function(){
 heading('סגירת חודש','אימות, תובנות והחלטה אחת לחודש הבא');
 let t=totals(selectedMonth),rows=[...monthTx(selectedMonth)].sort((a,b)=>(b.date||'').localeCompare(a.date||'')),unreviewed=rows.filter(x=>!x.reviewed),existing=db.reviews.find(r=>r.month===selectedMonth)||{},categoryRows=monthlyCategoryStats(selectedMonth),savedAssessments=new Map((existing.categoryAssessments||[]).map(x=>[x.category,x]));categoryRows.sort((a,b)=>Number(savedAssessments.get(a.id)?.completed===true)-Number(savedAssessments.get(b.id)?.completed===true));let assessedCount=categoryRows.filter(x=>savedAssessments.get(x.id)?.completed===true).length,steps=['סיכום פיננסי','אימות עסקאות','הערכת קטגוריות','רפלקציה'],body='',targets=stitchMonthlyTargets(selectedMonth),actualOut=t.expense+t.saving+t.debt,actualBalance=t.income-actualOut;
 ...
 if(reviewStep===3)body=`<article class="stitch-review-card stitch-category-review"><div class="stitch-section-title"><div><span class="stitch-kicker">הערכה אישית של החודש</span><h2>מה קרה בכל קטגוריה?</h2><p class="muted">בחר דירוג, הוסף הערה במידת הצורך ולחץ סיימתי. רק קטגוריה שסיימת תעבור למטה.</p></div><span class="stitch-category-review-progress">${assessedCount} מתוך ${categoryRows.length} הושלמו</span></div><form id="categoryAssessmentForm"><div class="stitch-category-assessment-grid">${categoryRows.length?categoryRows.map(row=>stitchCategoryAssessmentCard(row,savedAssessments.get(row.id)||{})).join(''):empty('category','אין קטגוריות פעילות','לא נמצאו יעדים או הוצאות לסיווג בחודש הזה')}</div><div class="stitch-review-actions"><button type="button" class="ghost" data-review-step="2">חזרה</button><button type="submit" class="primary">שמירת ההערכה והמשך</button></div></form></article>`;
 ...
 $$('.stitch-category-rating input').forEach(input=>input.onchange=()=>{let card=input.closest('[data-category-assessment]');if(input.checked)card.querySelectorAll('.stitch-category-rating input').forEach(other=>{if(other!==input)other.checked=false});card.dataset.completed='false';card.dataset.completionSource='';stitchSaveCategoryAssessment(card,categoryRows);stitchRefreshCategoryAssessment(card,categoryRows,false)});
 $$('.stitch-category-assessment-card textarea').forEach(textarea=>textarea.oninput=()=>{let card=textarea.closest('[data-category-assessment]'),state=card.querySelector('.stitch-category-save-state');if(state)state.textContent='שומר…';stitchSaveCategoryAssessment(card,categoryRows);stitchRefreshCategoryAssessment(card,categoryRows,false)});
 $$('[data-category-finish]').forEach(button=>button.onclick=()=>{let card=button.closest('[data-category-assessment]');if(!card.querySelector('.stitch-category-rating input:checked'))return toast('יש לבחור דירוג לפני שמסיימים');card.dataset.completed='true';card.dataset.completionSource='button';stitchSaveCategoryAssessment(card,categoryRows);stitchRefreshCategoryAssessment(card,categoryRows,true)});
 if($('#categoryAssessmentForm'))$('#categoryAssessmentForm').onsubmit=e=>{e.preventDefault();let record=db.reviews.find(r=>r.month===selectedMonth),currentAssessments=record?.categoryAssessments||[],categoryAssessments=$$('[data-category-assessment]').map(card=>{let category=card.dataset.categoryAssessment,stats=categoryRows.find(x=>x.id===category)||{},previous=currentAssessments.find(x=>x.category===category)||{},completed=card.dataset.completed==='true'&&card.dataset.completionSource==='button';return{...previous,category,rating:card.querySelector('.stitch-category-rating input:checked')?.value||'',note:card.querySelector('textarea').value.trim(),completed,completionSource:completed?'button':'',completedAt:completed?(previous.completedAt||new Date().toISOString()):'',target:stats.target||0,spent:stats.spent||0,difference:stats.difference||0,transactions:stats.transactions||0,budgetChange:stats.budgetChange||{count:0,net:0,increased:0,decreased:0}}});if(categoryAssessments.some(x=>!x.rating||!x.completed))return toast('יש ללחוץ סיימתי בכל קטגוריה');if(record)record.categoryAssessments=categoryAssessments;else db.reviews.push({month:selectedMonth,categoryAssessments});persist();reviewStep=4;render();toast('הערכת הקטגוריות נשמרה')};
