...
assert(d.querySelector('.stitch-forecast-heading > .stitch-forecast-icon')?.textContent.trim()==='monitoring','אייקון התחזית אינו נמצא בתא נפרד');
assert(!d.querySelector('.stitch-forecast-icon .material-symbols-outlined'),'אייקון התחזית עדיין עטוף בתוך אייקון נוסף');
...
assert(theme.includes('grid-template-areas:\"equation heading\"'),'אזורי התחזית במחשב אינם מופרדים');
assert(theme.includes('grid-template-columns:repeat(2,minmax(0,1fr))!important'),'קוביות התחזית בנייד אינן פרוסות באופן מאוזן');
...
console.log('HON Stitch v25 passed the clothing icon migration, isolated forecast icon, balanced desktop/mobile forecast layout, existing-data preservation, and all prior regression checks.');
